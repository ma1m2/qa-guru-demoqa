...
/*  public String state1 = fakerEn.options().option("NCR", "Haryana", "Rajasthan"),
  city1 = selectCity(state);

  public String selectCity(String state) {
    return switch (state) {
      case "NCR" -> fakerEn.options().option("Delhi", "Gurgaon", "Noida");
      case "Uttar Pradesh" -> fakerEn.options().option("Agra", "Lucknow", "Merrut");
      case "Haryana" -> fakerEn.options().option("Karnal", "Panipat");
      case "Rajasthan" -> fakerEn.options().option("Jaipur", "Jaiselmer");
      default -> null;
    };
  }*/
}
